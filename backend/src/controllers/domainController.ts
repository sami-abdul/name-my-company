...
import {
  checkDomainAvailability as checkDomainService,
  validateDomainName,
...
export const generateDomainSuggestions = async (
  ...
  try {
    ...
    // Use businessNiche as prompt if prompt is not provided (for frontend compatibility)
    const finalPrompt = prompt || businessNiche;

    if (!finalPrompt) {
      return res.status(400).json({
        status: "error",
        error: "Either 'prompt' or 'businessNiche' is required",
      } as ApiResponse<null>);
    }

    console.log("Generating domains for prompt:", finalPrompt);

    // Get user's actual subscription tier from middleware (set by usage enforcement)
    const userTier = (req as any).userTier || "free";

    // Select appropriate AI model based on actual tier or user preference
    const selectedModel: AIModel = model || getModelForTier(userTier);

    // Generate domains using enhanced AI service
    const aiResponse = await generateDomains(finalPrompt, {
      model: selectedModel,
      businessType: business_type || businessNiche,
      style: style || brandTone,
      keywords,
    });
    ...
    if (user_id) {
      try {
        ...
        console.log(
          `Saved ${domainSuggestions.length} domain suggestions for session ${sessionId}`
        );
      } catch (dbError) {
...
export const checkDomainAvailabilityController = async (
  req: Request,
  res: Response
): Promise<Response | void> => {
  try {
...
export const getUserGenerationHistory = async (
  req: Request,
  res: Response
): Promise<Response | void> => {
  try {
    ...
    const totalDomains = (sessions as any[]).reduce(
      (sum: number, session: any) =>
        sum + (session.domain_suggestions?.length || 0),
      0
    );
...
export const batchCheckDomainAvailability = async (
  req: Request,
  res: Response
): Promise<Response | void> => {
  try {
