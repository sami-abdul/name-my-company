...
async function testGetUserSessions(userId: string) {
  ...
  try {
    ...
    if (
      !session.domain_suggestions ||
      !Array.isArray(session.domain_suggestions)
    ) {
      throw new Error("Domain suggestions not properly included");
    }
    console.log(
      `✅ Domain suggestions included: ${session.domain_suggestions.length} domains`
    );
...
async function testValidationAndErrorHandling(userId: string) {
  ...
  try {
    ...
    try {
      ...
      if (
        error instanceof Error &&
        error.message.includes("Offset must be non-negative")
      ) {
...
async function testHistoryEndpointSimulation(user: TestUser) {
  ...
  try {
    ...
    const totalDomains = sessions.reduce(
      (sum, session) => sum + (session.domain_suggestions?.length || 0),
      0
    );
